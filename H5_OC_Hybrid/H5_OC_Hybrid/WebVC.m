...
- (void)viewDidLoad
{
    ...
    //添加一个按钮
    UIButton *backBtn = [[UIButton alloc]initWithFrame:CGRectMake(0.5*([[UIScreen mainScreen] bounds].size.width - 100), 35, 100, 50)];
    [backBtn setTitle:@"hello" forState:UIControlStateNormal];
    [backBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [backBtn addTarget:self action:@selector(hello) forControlEvents:UIControlEventTouchUpInside];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message
{
    ...
    if ([@"ocMethod" isEqualToString:messageName]) {
        ...
        UIButton *button = [[UIButton alloc]initWithFrame:CGRectMake(0.5*([[UIScreen mainScreen] bounds].size.width - 100),100,100,50)];
...
- (void)hello {
    ...
    //注意:js效果与第二种相同
//    [self.webView evaluateJavaScript:@"baidu()"completionHandler:^(id _Nullable result, NSError * _Nullable error) {
//
//    }];
}
